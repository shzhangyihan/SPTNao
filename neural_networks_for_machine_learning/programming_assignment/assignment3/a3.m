...
function ret = d_loss_by_d_model(model, data, wd_coefficient)
  ...
  ret.hid_to_class = error_deriv * hid_output';

  bp_deriv = model.hid_to_class' * error_deriv
  temp_deriv = exp(hid_input);
  ret.input_to_hid = (bp_deriv .* temp_deriv ./ ((temp_deriv + 1) .* (temp_deriv + 1))) * data.inputs';
  
  ret.input_to_hid = (1 / b_size) * ret.input_to_hid + model.input_to_hid * wd_coefficient;
  ret.hid_to_class = (1 / b_size) * ret.hid_to_class + model.hid_to_class * wd_coefficient;
end
